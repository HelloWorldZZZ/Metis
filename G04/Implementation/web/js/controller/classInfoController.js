...
$.get("api/web/getClassInfo.php", function(data) {
	...
	for (item in jsonArray) {
		//将科目加入下拉列表中
		$("#teacherDrop").append("<li><a href='#'>" + jsonArray[item].subject_name + "</a></li>");
	...
	$("#teacherDrop").append("<li class='divider'></li>");
	$("#teacherDrop").append("<li><a href='#'>" + "所有" + "</a></li>");

	window.localStorage.html = $("#table").html();

	$(document).ready(function() {
		$(".btn-group #teacherDrop li a").click(function() {
			$("tbody").empty();
			for (item in jsonArray) {
