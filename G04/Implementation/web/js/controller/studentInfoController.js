...
function processData(data, flag) {
	...
	for (item in jsonArray) {
		...
		if ($("#selector").html() == (jsonArray[item].class_no + "考场") || $("#selector").html() == "选择考场") {
			...
			var in_item;
			if (flag)
				$("#classDrop").append("<li><a href='#'>" + jsonArray[item].class_no + "考场" + "</a></li>");
	...
	if (flag) {
		$("#classDrop").append("<li class='divider'></li>");
		$("#classDrop").append("<li><a href='#'>" + "所有" + "</a></li>");
	}
	
	window.localStorage.html = $(".table").html();

	$(document).ready(function() {
		$("#classDrop li a").click(function() {
